...
class SoftMax(BaseLayer):
    ...
    def forward(self, input_tensor):
        """
        Compute the forward pass for the SoftMax layer.

        Parameters:
            input_tensor (np.ndarray): The input logits of shape (batch_size, num_classes).

        Returns:
            np.ndarray: Probability distribution for each row, of shape (batch_size, num_classes).
        """      
